...
def plant(graph, skeleton, dictionary=None):
    ...
    if typ == "tree":
        ...
        _plant_children(graph, skeleton["data"], category,
                        ...
                        dictionary=dictionary, seen=seen)
    elif typ == "flat":
        for name in skeleton["data"]:
            _ensure_node(graph, name, category, dictionary, seen)
    else:
        raise ValueError(
...
def _plant_children(graph, children_spec, category, child_rel, sibling_rel,
    ...
    for name, subtree in items:
        nid = _ensure_node(graph, name, category, dictionary, seen)
        ...
        if subtree:
            _plant_children(graph, subtree, category, child_rel, sibling_rel,
                            parent_id=nid, dictionary=dictionary, seen=seen)
...
def _ensure_node(graph, canonical, category, dictionary, seen):
