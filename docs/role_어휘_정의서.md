...
>
> v1.8: 명세 v1.12 동기화 — §3.1 anchor **Tier1 한정**(auto 후보뿐이면 orphan_anchor)·극성 모호 처리,
> §3.2 entity에 **canonical 스코프**(Property 등 부모 접두 — 명세 §5.2) 및 리스트 값 계약 위반 시 missing_field 큐.
...
- **리스트 값은 계약 위반 (v1.12)**: entity 필드에 리스트/구조 값이 오면(파서가 전개하지 않음 — §3.4 리스트 값 처리·§12-3) 값을 문자열로 뭉개 노드를 만들지 않고 **드롭 + `missing_field` 큐**. 핸들러는 단일 값만 본다는 전제를 코드가 방어한다.
